...
**项目结构**（在 smile-max-server 中扩展）：

```
smile-max-server/src/
...
| Recipe ID | 标题 | 来源 |
|-----------|------|------|
| `ui-components` | SwiftUI 组件集 | slComponent/ 20+ 个文件整合 |
| `animations` | 动画组件集 | slAnimation/ 9 个文件整合 |
| `onboarding` | Onboarding 引导流程 | slOnboardingView 提取 |

**付费 Recipe（MVP 首批）**：

| Recipe ID | 标题 | 来源 |
|-----------|------|------|
| `auth-cognito` | 认证系统 | slUserManager (644行) + 1_auth.md (1,737行) |
| `subscription-storekit` | 订阅系统 | slStoreManager (107行) + slPaywallView + 3_subscription.md (841行) |
| `infra-cdk` | AWS 基础设施 | 0_cdk.md (1,041行) + CDK constructs |

**Recipe 格式**：遵循第三章定义的 frontmatter + 固定章节（解决什么问题 → 架构决策 → 依赖 → 实现 → 集成清单 → 常见定制 → 已知陷阱）
...
- 验证：`claude mcp add --transport http shipswift https://api.shipswift.dev/mcp`

### 阶段四：数据库 + License 校验

**目标**：Recipe 迁移到 Aurora，加入付费访问控制
...
- Twitter / 独立开发者社区推广
- `listRecipes` 的 tool description 动态生成（从数据库实时拼接关键词）

### 建议执行顺序

**阶段一 → 阶段二 → 本地验证 → 阶段三 → 阶段四 → 阶段五**

理由：先跑通 MCP 协议（阶段一），再填充内容（阶段二），本地验证 OK 后尽快部署（阶段三），数据库和 License 可以后加（阶段四），因为早期不需要付费校验。
...
| 产品 | 最低价 | 最高价 | 模式 |
|------|-------|-------|------|
| ShipiOS | $79 | $999 | 多档位（功能分级） |
| iOS App Templates | $99 | $249 | 按模板单卖 |
| SwiftyLaunch | $179 | $229 | 平台分级 |
| Swift Starter Kits | $199 | $199 | 单一定价 |
| WrapFast | $299 | $4,999 | 标准版 + 企业版 |
| ShipThatApp | $269 | $4,499 | 模板 + 定制开发 |

**市场均价**：约 $199（通用 Starter Kit）
...
- [SwiftyLaunch](https://swiftylaunch.com/)
- [WrapFast](https://wrapfa.st/)
- [Swift Starter Kits](https://swiftstarterkits.com/)
- [ShipThatApp](https://www.shipthat.app/)
- [SwiftShip](https://swiftship.dev/)
- [ShipiOS](https://www.shipios.app/)
- [Starter Index](https://starterindex.com/swift-boilerplates)
- [BoilerplateSearch](https://boilerplatesearch.com/iOS)
- [Scrapbook](https://www.getscrapbook.com/boilerplates/swiftui)
